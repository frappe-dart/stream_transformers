...
void main() => describe("CombineLatest", () {
...
void testWithStreamController(StreamController providerA()) {
  ...
  it("combine when both streams have an event", () {
    return testStream(controllerA.stream.transform(new CombineLatest(controllerB.stream)),
        behavior: () {
          controllerA.add(1);
          controllerB.add(1);
        },
  ...
  it("combines always after both streams have an event", () {
    return testStream(controllerA.stream.transform(new CombineLatest(controllerB.stream)),
        ...
        expectation: (values) => expect(values).toEqual([[1, 1], [2, 1], [2, 2]]));
  });
  ...
  it("cancels input streams when source streams are closed", () {
    ...
    return testStream(
        controllerA.stream.transform(new CombineLatest(controllerB.stream)),
        behavior: () {
          controllerA.close();
          controllerB.close();
        },
  ...
  it("returns a stream of the same type", () {
    var stream = controllerA.stream.transform(new CombineLatest(controllerB.stream));
